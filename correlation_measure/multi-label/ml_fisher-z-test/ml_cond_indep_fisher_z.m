function [CI, r, p] = ml_cond_indep_fisher_z(data, f, L, S, N, alpha)
...
%   L    : the index set of the label set
%   S    : ???
...
%C     = cov([data(:,[X,Y,S])]);  %Commented by Liu

C      = cov(data(:, [f, L, S])); %Add by Liu
size_C = size(C, 2);
X1     = 1;
Y1     = 2;
S1     = 3 : size_C; %Add by Liu

%S1    = [3:size_C]; %Commented by Liu
...
%[errorcode x mu sigma] = distchck(3,x,mu,sigma);      %Commented by Liu
[errorcode, x, mu, sigma] = distchck(3, x, mu, sigma); %Add by Liu
...
%[errorcode p mu sigma] = distchck(3, p, mu, sigma);   %Commented by Liu
[errorcode, p, mu, sigma] = distchck(3, p, mu, sigma); %Add by Liu
